...
summary_multivar <- function(data, group_name) {
  ...
  info1 <- data_clean %>% group_by(gene) %>%
    summarise(cum_si = sum(selection_intensity),
...
size_merge <- size_merge %>%
  mutate(n_variant = paste(as.character(n_variant.small), 
         ...
         cum_si_total = round(cum_si_total/1e4, 1)
  )

size_merge_sub <- size_merge %>%
  select("gene", "n_variant", "cum_si_total", "topvar") %>%
  arrange(desc(cum_si_total)) %>%
  mutate(format = "Small Size | Large Size")

names(size_merge_sub) <- c("gene", "#variant", "total SI (10^4)", "TopVariant", "format")
write.csv(size_merge_sub, file = "gene_multivar_summary.csv", quote = F, row.names = F)
...
stage_merge <- merge(stage1_info, stage234_info, by = "gene", all = T, 
  ...
  mutate(cum_si_total = cum_si.1 + cum_si.234) %>%
  arrange(desc(cum_si_total))
...
names(stage_merge_sub) <- c("gene", "#variant", "total SI (10^4)", "TopVariant", "format")
write.csv(stage_merge_sub, file = "stage_gene_multivar_summary.csv", quote = F, row.names = F)
